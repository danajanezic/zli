...
program.on('option:develop', () => {
  global.vn.RUNTIME_FLAGS.push(RUNTIMES.DEVELOP);
});

program.on('option:local', () => {
  global.vn.RUNTIME_FLAGS.push(RUNTIMES.LOCAL);
});

program.on('option:staging', () => {
  global.vn.RUNTIME_FLAGS.push(RUNTIMES.STAGING);
});

program.on('option:prod', () => {
  console.warn(chalk.yellowBright('WARNING: running with production flag toggled'));
  global.vn.RUNTIME_FLAGS.push(RUNTIMES.PRODUCTION);
});
...
async function withZliCmd(callback, ...commandPath) {
  await within(async () => {
    const filePath = commandPath[commandPath.length - 1].includes('.js')
      ? [process.cwd(), 'cli', 'commands', ...commandPath]
      : [process.cwd(), 'cli', 'commands', ...commandPath, 'index.js'];
...
export function registerGlobals(config = {}) {
  Object.assign(global, {
    ...
    die: (...args) => {
      console.error(fmtErrStr(args.join(' ')));
      process.exit(1);
    },
    ifFunc:
      func =>
...
async function importPath(filepath, args, command) {
  ...
  const useOpts = (callback, ...argStrings) => {
    ...
    if (argStrings.length === Object.keys(actualArgs).length) {
      return zx.within(() => callback({ ...actualArgs, ...otherArgs }, otherArgs));
    }
  };

  const getConfigVars = async app => {
    const { stdout } = await $`heroku config -a ${app} --json`;
  ...
  const whenRuntime = (...runtimes) => {
    return (callback, onDefault) => {
      ...
      return filtered.length ? ifFunc(callback)(filtered, getConfigVars) : ifFunc(onDefault)(getConfigVars);
    };
  };
...
const defaultCommanderAction = OPTS => filepath => {
  return async (args, command) => {
    if (OPTS.showHelpWhenNoArgs && !vn.hasValues(args)) {
      command.help();
    }

    if (OPTS.requiresRunTime && !vn.RUNTIME_FLAGS.length) {
      if (OPTS.requiresRunTime.default) {
        vn.RUNTIME_FLAGS.push(OPTS.requiresRunTime.default);
      } else {
    ...
    if (OPTS.options) {
      ...
      try {
        ...
        if (vn.hasValues(validationErrors)) {
...
export function createInterpreter(scriptPath, argv, extraOpts, doDebug = false) {
