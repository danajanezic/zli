...
export default async function () {
  ...
  const config = {};
  const commandRoot =
    (await question(`Root path for ZLI? (default: zli-commands)`)) || 'zli-commands';
  config.root = `${__dirname}/${commandRoot}/`;
  config.commandExecutable =
    (await question(`Name of executable to run ZLI? (default: zli)`)) || 'zli';
  const proceed =
    (await question(`Proceed with config: ${JSON.stringify(config, null, 4)}? (y/N)`)) || 'N';
  ...
  if (fs.existsSync(config.root)) {
    ...
    if (overwriteRoot.toLocaleLowerCase().startsWith('y')) {
      ...
      fs.copyFileSync(
        `./node_modules/@venicemusic/zli/bin/commands/default-opts.js`,
        config.root + '/index.js'
      );
      fs.copyFileSync(
        `./node_modules/@venicemusic/zli/bin/commands/show-globals.js`,
        config.root + '/show-globals.js'
      );
      fs.copyFileSync(
        `./node_modules/@venicemusic/zli/bin/commands/globals.md`,
